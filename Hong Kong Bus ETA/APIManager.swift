...
enum API {
    ...
    var url : URL {
        ...
        case .CTBRouteData(let route, let isInbound):
            urlString =  "https://rt.data.gov.hk/v1/transport/citybus-nwfb/route-stop/ctb/\(route)/\(isInbound ? "inbound" : "outbound")/"
        case .KMBRouteData(let route,let isInbound, let serviceType):
            urlString =  "https://data.etabus.gov.hk/v1/transport/kmb/route-stop/\(route)/\(isInbound ? "inbound" : "outbound")/\(serviceType)"
        ...
        case .KMBArrivalEstimation(let stopId, let route, let serviceType):
            urlString = "https://data.etabus.gov.hk/v1/transport/kmb/eta/\(stopId)/\(route)/\(serviceType)"
        }
    ...
    var header : [HTTPHeader] {
        ...
        let dict : [String:String] = switch self {
        case .CTBRoutes, .KMBRoutes, .CTBRouteData, .KMBRouteData, .CTBBusStopDetail, .KMBBusStopDetail, .CTBArrivalEstimation, .KMBArrivalEstimation:
             [:]
...
class APIManager : APIManagerType {
    ...
    func call(api : API) -> AnyPublisher<Data?, Error> {
        ...
        return Future { promise in
            ...
            Task {
                let request = AF.request(api.url,method: HTTPMethod(rawValue: self.getMethod(forAPI: api)), parameters: api.parameter, headers: HTTPHeaders(api.header))
