  ...
  func filterByKeywords(_ keywords: [String]) -> [Element] {
    ...
    return self.filter { busRoute in
      ...
      guard
        ...
        let destinationEn = busRoute.destinationEn?.lowercased()
      else {
      ...
      for keyword in keywords {
        ...
        if route.contains(keyword) || originEn.contains(keyword) || originSC.contains(keyword)
          || originTC.contains(keyword) || destinationEn.contains(keyword)
          || destinationSC.contains(keyword) || destinationTC.contains(keyword)
        {
          continue
        }
